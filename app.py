...
DHAN_ACCESS_TOKEN = "import streamlit as st
import requests
import pandas as pd
import plotly.graph_objects as go
...
def dhan_ltp(security_id: int) -> float:
    url = f"{BASE_URL}/v2/marketdata/ltp"
    payload = {"securities": {"NSE_EQ": [security_id]}}
    r = requests.post(url, headers=HEADERS, json=payload, timeout=10)
    r.raise_for_status()
    return r.json()["data"]["NSE_EQ"][str(security_id)]["ltp"]
...
st.set_page_config(
    page_title="NYZTrade – Valuation Dashboard",
    layout="wide"
)
...
eps = st.sidebar.number_input("EPS (TTM)", value=50.0, step=1.0)
growth_rate = st.sidebar.number_input("Expected Growth (%)", value=12.0, step=0.5)
discount_rate = st.sidebar.number_input("Discount Rate (%)", value=10.0, step=0.5)
industry_pe = st.sidebar.number_input("Industry PE", value=20.0, step=1.0)
...
fig.add_bar(name="PE Fair Value", x=["PE Fair Value"], y=[fair_value_pe])
fig.add_bar(name="DCF Value", x=["DCF Value"], y=[dcf_value])
